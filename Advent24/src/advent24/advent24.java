...
public class advent24 {
	...
	public static void main(String[] args) throws IOException {
		...
		// print();
		// System.exit(0);
		System.out.println("Max str= " + calcMaxStrenght(comp, "", 0, true));
	}

	static int calcMaxStrenght(List<String> inA, String bridge, int strenght, boolean frontPortUsed) {
		...
		if (bridge.equals("")) {
			for (int i = 0; i < inA.size(); i++) {
				if (inA.get(i).charAt(0) == '0') {
					...
					int maxv = calcMaxStrenght(v, bridge + "--" + del, val + strenght, true);
			...
			for (int i = 0; i < inA.size(); i++) {
				...
				if (anslut == Integer.valueOf(ports[1])) {
					...
					String del = v.remove(i);
					int val = compValue(del);
					int maxv = calcMaxStrenght(v, bridge + "--" + del, val + strenght, false);
					maxStyrka = Math.max(maxStyrka, maxv);
				}
			}
		}
