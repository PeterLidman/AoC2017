...
//		 }
	...
	static int countRecursive(int iterations, String image) {
		...
		String tmp ="";
		System.out.println("iter=" + iterations + " image= " + image);
		...
		if (iterations < 1) { // count # i ett löv
			...
			System.out.println("räknade #=" + retur + " image= " + image);
			return retur;
		}
		...
		if (image.length() > 9) {
			...
			tmp="";
			if (image.length() % 2 == 0) {
				System.out.println("%2 side :" + image );
				for (int i = 0; i < side; i += 2) {
					for (int j = 0; j < side; j += 2) {
					tmp +=	fullBok.get(image.substring(j+i*side, j+i*side+2) +	image.substring(j+(i+1)*side, j+(i+1)*side+2));
					}
				}
				System.out.println(tmp);
				return countRecursive(iterations - 1,tmp);
			} else { // 3
				System.out.println("%3 side :" + image );
				for (int i = 0; i < side; i += 3) {
					for (int j = 0; j < side; j += 3) {
						tmp +=	fullBok.get(image.substring(j+i*side, j+i*side+3) +	image.substring(j+(i+1)*side, j+(i+1)*side+3) +	image.substring(j+(i+2)*side, j+(i+2)*side+3));
					}
				}
				System.out.println(tmp);
				return countRecursive(iterations - 1,tmp);
